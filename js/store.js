...
const KEYS = {
  ...
  CUSTOM_EXERCISES: "myberk_custom_exercises_v1" // user-added exercises, merged into Library at load time
};
...
const Store = {
  ...
  // --- Custom exercises (user-added, on top of the static library) ---
  getCustomExercises() {
    return readJSON(KEYS.CUSTOM_EXERCISES, []);
  },
  addCustomExercise(exercise) {
    const list = this.getCustomExercises();
    list.push(exercise);
    writeJSON(KEYS.CUSTOM_EXERCISES, list);
    return exercise;
  },
  ...
  deleteCustomExercise(id) {
    writeJSON(KEYS.CUSTOM_EXERCISES, this.getCustomExercises().filter((e) => e.id !== id));
  },
...
// --- Exercise library (static dataset + user-added custom exercises) ---
const Library = {
  exercises: [],
  loaded: false,
  async load() {
    ...
    this.exercises = base.concat(Store.getCustomExercises());
    this.loaded = true;
  ...
  // Adds a user-created exercise to both persistent storage and the
  // in-memory list, so it shows up in the Library immediately without
  // re-fetching the (large, static) exercises.json.
  addCustom(exercise) {
...
// Returns true if a session on the given calendar date (dateStr, "YYYY-MM-DD")
// starting at startTime ("HH:MM") for durationMinutes overlaps the 9am-5pm
// weekday work block. Weekday is derived from the date itself (rolling
// calendar - sessions no longer carry a fixed dayOfWeek).
function conflictsWithWork(dateStr, startTime, durationMinutes) {
  const jsDay = new Date(dateStr + "T00:00:00").getDay(); // 0=Sun..6=Sat
  const dow = (jsDay + 6) % 7; // 0=Mon..6=Sun
  if (dow > 4) return false; // Sat/Sun - no work conflict
...
// ---------- one-time migration: recurring weekly template -> rolling calendar ----------
// The schedule used to be a single recurring weekly template (sessions
// carried a fixed dayOfWeek and repeated forever). It's now a real rolling
// calendar where every session has its own date. This runs once: any
// leftover dayOfWeek-only sessions get materialized onto THIS week's actual
// dates, then the migration is marked done so it never runs again (so
// re-adding a Monday workout later doesn't get re-migrated).
const CALENDAR_MIGRATED_KEY = "myberk_calendar_migrated_v1";

function migrateRecurringScheduleToCalendar() {
  if (localStorage.getItem(CALENDAR_MIGRATED_KEY)) return;
  ...
  let changed = false;
  sessions.forEach((s) => {
    if (s.date) return; // already a dated session, nothing to do
    if (typeof s.dayOfWeek !== "number") return;
  ...
  if (changed) Store.saveSessions(sessions);
  localStorage.setItem(CALENDAR_MIGRATED_KEY, "1");
}
